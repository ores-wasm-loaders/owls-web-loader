...
fn build(recipe: &Recipe) -> Result<Value> {
    if !identifier(&recipe.app_id) || !identifier(&recipe.release) || !safe_path(&recipe.entrypoint) { return Err("invalid build identity".into()); }
    ...
    Ok(json!({"schemaVersion": 2, "appId": recipe.app_id, "release": recipe.release, "runtime": recipe.runtime,
        ...
        "prepareBudget": {"maxBytes": 1048576, "furthestStage": "fetch"},
        "activation": if recipe.runtime == "flutter-web" { json!({"mode": "attach-view"}) } else { json!({"mode": "hydrate-islands", "islands": recipe.islands}) },
        "config": {"buildTool": "owls-build-manifest/0.1.0", "toolchain": recipe.toolchain, "contentTypes": content_types, "publicStaticAssetsOnly": true}}))
}
fn main() {
    let run = || -> Result<()> {
...
mod tests {
    ...
    #[test] fn identifiers_cannot_form_paths() { assert!(!identifier("../r1")); assert!(!identifier("")); assert!(identifier("r1-abcdef")); }
