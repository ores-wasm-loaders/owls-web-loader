...
## HTML-first marketing sites

Annotate only links whose destination release is a realistic next action. The shared integration
fetches the public manifest after meaningful intent, then acquires the coordinator's existing
fetch-only preparation lease. It never prevents the click, imports an entrypoint, or starts a
runtime, so failed or skipped preparation falls through to ordinary navigation.

```html
<a
  href="https://app.example.com/"
  data-owls-manifest="https://assets.example.com/releases/app/2026.09.06-a/loader-manifest.json"
>
  Open app
</a>
<script type="module">
...
variants. A full-page destination still initializes its own runtime; response-cache reuse is a
measured optimization, not a cross-document object registry.
