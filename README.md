...
Preparation never executes JS or WASM. It checks declared budgets before requests, limits streaming reads, verifies exact sizes and SHA-256 hashes, and shares one cancellable preparation job across all leases, including callers with explicit signals. `prefetch` resolves a `PreparationOutcome`; `warmed`, `failed`, `cancelled` and policy-`skipped` are preparation results, not activation failures. `prepareOnIntent` applies 150 ms dwell/exit-grace defaults and releases unclaimed work on page lifecycle changes. Failed warmup does not prevent demand loading.

Use one Coordinator and one adapter object per release in a retained document. Activation claims speculative preparation but joins it only for the bounded `activationJoinMs` policy (50 ms by default), then follows its normal demand path. An activation failure is terminal for that owner because initialization may have partially executed. `deactivate` invokes optional adapter cleanup before a deliberate restart. Recreate the host/document only after application-specific cleanup; a timeout rejects the caller but cannot undo a non-cooperative adapter's side effects. RawWasmAdapter supports optional compilation without instantiation through compile(context). BindgenAdapter accepts the exact generated glue, binary asset ID and application start hook. LeptosAdapter accepts a pinned hydrate hook; DioxusAdapter leaves routing and splitting to Dioxus. Do not call __wbindgen_start yourself.
